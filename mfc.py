...
def read_model_data(m):
   ...
   vs = msg['vs']
   if vs == 127:
      print (colored(" => ({}) is OFFLINE <=", "yellow", "on_red")).format(camgirl)
      print(colored("\n => END <=", 'yellow','on_blue'))
      time.sleep(3)
      sys.exit()
   ...
   print (colored("\n => Occupation: {} * New model: {} * Blurb: {} <=", "yellow", "on_blue")).format(occupation,newmodel,blurb)
...
if __name__ == "__main__":
   ...
   try:
      ...
      print (colored(" => {} server is busy ... Try again <=", "yellow", "on_red")).format(cserver)
      print(colored("\n => END <=", "yellow","on_blue"))
      time.sleep(6)
      sys.exit()
   rembuf = ''
   ...
   if vs == 0:
      if server != 0:
         ...
         fn4 = camgirl + '_MFC_' + timestamp + '.txt'
         pf1 = (path + fn1)
         pf2 = (path + fn2)
         pf3 = (path + fn3)
         pf4 = (path + fn4)
         ...
         print (colored(" => ({}) is 'NO MOBILE FEED' model who isn't supported yet <=", "yellow", "on_red")).format(camgirl)
         raw_input(colored("\n => Press Enter to exit <=", "yellow", "on_blue"))
         print(colored("\n => END <=", "yellow","on_blue"))
         time.sleep(1)
         sys.exit()
   else:
      print (colored(" => ({}) video stream can't be recorded now <=", "yellow", "on_red")).format(camgirl)
      raw_input(colored("\n => Press Enter to exit. <=", "yellow", "on_blue"))
      print(colored("\n => END <=", "yellow","on_blue"))
      time.sleep(1)
      sys.exit()
