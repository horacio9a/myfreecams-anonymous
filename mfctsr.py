# MFC Remote FF-TS Anonymous Recorder v.1.1.0 by horacio9a for Python 2.7.14
...
if __name__ == '__main__':
   ...
   if vs == 0:
      if server != 0:
         ...
         filename = model + '_MFC_' + timestamp + '.ts'
         pf = path + filename
         ffmpeg = config.get('files', 'ffmpeg')
         print
         print (colored(' => FF-TS-REC => {} <=','yellow','on_red')).format(filename)
         print
         command = '{} -hide_banner -loglevel panic -i {} -map 0:1 -map 0:2 -c copy -vsync 2 -r 60 -b:v 500k {}'.format(ffmpeg,hlsurl,pf)
         os.system(command)
         print(colored(" => END <=","yellow","on_blue"))
         sys.exit()
